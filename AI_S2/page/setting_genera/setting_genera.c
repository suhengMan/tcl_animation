#include "lvgl.h"
#include "setting_genera.h"
#include "setting_genera_view.h"
#include "cl_ui.h"

static setting_genera_t pg;
...
{   
    pg.setting_genera_view = setting_genera_view_create(self->root);
}   
...
static void on_view_did_appear(page_base_t *self) {
    pg.setting_genera_view->is_act = 1;
}
static void on_view_will_disappear(page_base_t *self) { pg.setting_genera_view->is_act = 0; }
static void on_view_did_disappear(page_base_t *self) {}
static void on_view_did_unload(page_base_t *self) { setting_genera_view_delete(); }
