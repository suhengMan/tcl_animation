#include "lvgl.h"
#include "cl_ui.h"
#include "pg_phone_call.h"
#include "vw_phone_call.h"

static phone_call_pg_t pg;
...
{
    // phone_call_model_init(); // 如有模型初始化可放开
    char phone[32] = {0};
    page_get_stash(self, phone, sizeof(phone));
    pg.view = phone_call_view_create(self->root, phone);
}
