 ...
 * @return page_base_t* 页面对象
*/
page_base_t *get_stack_top_after_by_index(page_manager_t *self,size_t index){
    listNode *base_node = listIndex(self->page_stack, index);
    if (base_node == NULL)
    {
        PM_LOG_ERROR("page_manage is empty or only heve one page");
        return NULL;
...
const char* pm_get_last_page_name(page_manager_t *self){
    return get_stack_top_after(self)->name;
}

const char* pm_get_last_page_name_by_index(page_manager_t *self, size_t index){
    page_base_t* last_page = get_stack_top_after_by_index(self, index);
    if(last_page ==NULL){
        return NULL;
    }
    return last_page->name;
}

page_base_t *pm_find_page(page_manager_t *self, const char *name)
{
    return find_page_pool(self, name);
}