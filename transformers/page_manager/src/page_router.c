...
static void _page_switch(page_manager_t *self, page_base_t *new_node, bool is_push_act, const page_stash_t *stash)
{
    ...
    if (stash != NULL) // 如果有缓存区
    {
        ...
        if (new_node->priv.stash.ptr == NULL)
        {
            ...
            {
                PM_LOG_INFO("stash(%p) malloc[%ld]", buffer, stash->size);
            }
        }
        ...
        if (buffer != NULL)
        {
            ...
            PM_LOG_INFO("stash memcpy[%ld] %p >> %p", stash->size, stash->ptr, buffer);
...
void pm_pop_until(page_manager_t *self, const char *name) {
  while (1) {
    ...
    if (top_after == NULL) {
      fource_unload(top);
      listDelNode(self->page_stack, listSearchKey(self->page_stack, top));
      pm_push(self, name, NULL);
      break;
    }
    if (strcmp(top_after->name, name) == 0) {
      pm_pop(self);
      break;
    }
    fource_unload(top);
